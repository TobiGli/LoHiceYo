...
const db = require('../db');
const { computeTotals, isLastDayOfMonth, monthKeyOf } = require('../scheduler');
...
router.get('/', (req, res) => {
  ...
  const finalResult = db.prepare('SELECT * FROM monthly_results WHERE month = ?').get(month);
...
router.get('/results', (req, res) => {
  const rows = db.prepare('SELECT * FROM monthly_results ORDER BY month DESC').all();
  res.json(rows);
});
